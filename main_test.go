...
func TestPipeline(t *testing.T) {
	...
	freeFlowJobs := []job{
		job(func(in, out chan interface{}) {
			...
			// тут реализована проверка, что счетчик увеличился в следующей функции
...
func TestSigner(t *testing.T) {
	...
	// здесь переопределены фукции из файла common.go. Эти функции инкрементят локальный счетчик
	// переопределение возможо потому что функции объявлены как переменные
